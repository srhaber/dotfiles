...
function initComponent(componentDef, doc) {
    ...
    var isExisting = componentDef._n_;
    var id = component.id;

    var rootIds = componentDef.s_;
    ...
    if (rootIds) {
        ...
        rootIds.forEach(function (rootId) {
            ...
            if (rootComponent) {
                rootComponent.i_ = component;
                if (rootComponents) {
                    rootComponents.push(rootComponent);
                } else {
                    rootComponents = component.H_ = [rootComponent];
                }
            } else {
    ...
    var domEvents = componentDef._m_;
    if (domEvents) {
        ...
        if (eventListenerHandles.length) {
            component.u_ = eventListenerHandles;
        }
    }
...
function initClientRendered(componentDefs, doc) {
    ...
    for (var i = 0, len = componentDefs.length; i < len; i++) {
        ...
        if (componentDef._l_) {
            initClientRendered(componentDef._l_, doc);
        }
...
function initServerRendered(renderedComponents, doc) {
    ...
    // Ensure that event handlers to handle delegating events are
    // always attached before initializing any components
    eventDelegation._O_(doc || defaultDocument);
    ...
    componentDefs.forEach(function (componentDef) {
        componentDef = ComponentDef._u_(componentDef, typesArray, serverRenderedGlobals, registry);
        initComponent(componentDef, doc || defaultDocument);
    });
}

exports._A_ = initClientRendered;
exports._L_ = initServerRendered;