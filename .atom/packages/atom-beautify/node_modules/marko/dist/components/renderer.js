...
function resolveComponentKey(globalComponentsContext, key, scope) {
    if (key[0] == '#') {
        ...
        if (repeatedRegExp.test(key)) {
            resolvedId = globalComponentsContext._s_(scope, key);
        } else {
...
function preserveComponentEls(existingComponent, out, globalComponentsContext) {
    var rootEls = existingComponent.V_({});
    ...
    for (var elId in rootEls) {
        ...
        globalComponentsContext._C_(elId); // Mark the element as being preserved (for morphdom)
    }

    existingComponent.l_(); // The component is no longer dirty so reset internal flags
    return true;
}
...
function handleBeginAsync(event) {
    ...
    var componentsContext = parentOut.data._B_;
    ...
    if (componentsContext !== undefined) {
        ...
        // parent block.
        var nestedComponentsContext = componentsContext._D_(asyncOut);
        asyncOut.data._B_ = nestedComponentsContext;
    }
    // Carry along the component arguments
    asyncOut._X_ = parentOut._X_;
}
...
function createRendererFunc(templateRenderFunc, componentProps, renderingLogic) {
    ...
    return function renderer(input, out) {
        ...
        var component = globalComponentsContext.Y_;
        ...
        if (component) {
            id = component.id;
            isExisting = true;
            globalComponentsContext.Y_ = null;
        } else {
            var componentArgs = out._X_;
        ...
        id = id || componentsContext._t_();

        if (isServer) {
            component = registry._v_(renderingLogic, id, input, out, typeName, customEvents, scope);
            input = component._F_;
            component._F_ = undefined; // We don't want ___updatedInput to be serialized to the browser
        } else {
            if (!component) {
                if (isRerender) {
                    // Look in in the DOM to see if a component with the same ID and type already exists.
                    component = componentLookup[id];
                    if (component && component.g_ !== typeName) {
                        component = undefined;
                    }
                }
                ...
                if (component) {
                    ...
                    component = registry._v_(typeName, id);
                ...
                if (customEvents !== undefined) {
                    component._f_(customEvents, scope);
                }
                ...
                input = component.O_(input, onInput, out);

                if (isExisting === true) {
                    if (component.Q_ === false || component.shouldUpdate(input, component.r_) === false) {
            ...
            component.A_ = outGlobal;

            emitLifecycleEvent(component, 'render', out);
        }

        var componentDef = componentsContext._E_(component, isSplit);
        componentDef.s_ = roots;
        componentDef._n_ = isExisting;

        // Render the template associated with the component using the final template
        // data that we constructed
        templateRenderFunc(input, out, componentDef, component, component._d_);
...
createRendererFunc._U_ = resolveComponentKey;
createRendererFunc._V_ = preserveComponentEls;
